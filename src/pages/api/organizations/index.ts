...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.guided_selling_solution?.length > 0) {
      const create_guided_selling_solution = body.guided_selling_solution;
      body.guided_selling_solution = {
        create: create_guided_selling_solution,
      };
    } else {
      delete body.guided_selling_solution;
    }
